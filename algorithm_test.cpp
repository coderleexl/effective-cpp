...
TEST(Algorithm, FindTargetForMatrix)
{
	// 测试用例1：目标值存在
	...
	ASSERT_TRUE(Algorithm::FindTargetForMatrix(matrix1, 5) == true);
	...
	ASSERT_TRUE(Algorithm::FindTargetForMatrix(matrix2, 10) == false);

	// 测试用例3：空矩阵
	std::vector<std::vector<int>> matrix3;
	ASSERT_TRUE(Algorithm::FindTargetForMatrix(matrix3, 5) == false);

	// 测试用例4：只有一行的矩阵
	std::vector<std::vector<int>> matrix4 = {
		{1, 3, 5}
	};
	ASSERT_TRUE(Algorithm::FindTargetForMatrix(matrix4, 3) == true);
